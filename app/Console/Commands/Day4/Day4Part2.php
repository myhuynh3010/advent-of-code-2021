...
class Day4Part2 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'day4:part2';
    ...
    public function handle()
    {
        ...
        for ($i=0; $i < count($numbers); $i++) { //check one number at the time
            ...
            // create new array for non winning boards
            $remainingBoards = [];
            ...
            foreach ($boards as $n => $board) {
                if ($this->isWinner($drawns, $board)) {
                    ...
                    // keep it in non winning boards
                    $remainingBoards[] = $board;
                }
            }
            // reset boards to non winning boards
            if (count($remainingBoards) !== 0) {
